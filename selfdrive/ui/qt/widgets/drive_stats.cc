...
DriveStats::DriveStats(QWidget* parent) : QFrame(parent) {
  ...
  auto add_stats_layouts = [=](const QString &title, StatsLabels& labels) {
    ...
    int row = 0;
    grid_layout->addWidget(newLabel(title, "〔  누적 주행기록  〕 \U00002728"), row++, 0, 1, 3);
    ...
    grid_layout->addWidget(labels.hours = newLabel("0", "number"), row, 1, Qt::AlignLeft);
    grid_layout->addWidget(labels.distance = newLabel("0", "number"), row, 2, Qt::AlignLeft);

    grid_layout->addWidget(newLabel("Drives", "unit"), row + 1, 0, Qt::AlignLeft);
    grid_layout->addWidget(newLabel("Hours ", "unit"), row + 1, 1, Qt::AlignLeft);
    grid_layout->addWidget(labels.distance_unit = newLabel(getDistanceUnit(), "unit"), row + 1, 2, Qt::AlignLeft);
